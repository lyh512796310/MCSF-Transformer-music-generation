...
def normal_embedding_lookup(x, n_token, d_embed, d_proj, initializer,
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        if d_proj != d_embed:
            ...
            y = tf.einsum('ibe,ed->ibd', y, proj_W) # 矩阵连乘
        else:
            proj_W = None
...
def normal_softmax(hidden, target, n_token, params, scope='normal_softmax', **kwargs):
    def _logit(x, W, b, proj):
        y = x
        if proj is not None:
            y = tf.einsum('ibd,ed->ibe', y, proj) # 矩阵
...
def positional_embedding(pos_seq, inv_freq, bsz=None):
    ...
    pos_emb = tf.concat([tf.sin(sinusoid_inp), tf.cos(sinusoid_inp)], -1) # 连接
    if bsz is not None:
...
if __name__ == '__main__':
    ...
    # a = np.asarray(a)
    # id = [[0, 2, 1], [1, 1, 2]]
    # out = tf.nn.embedding_lookup(a, id)
